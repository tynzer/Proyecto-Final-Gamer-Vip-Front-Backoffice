    ...
    filter(e) {
        ...
        if (data) {
            ...
            const newData = banners.filter(function (item) {
                const itemDataTitle = item.titulo.toUpperCase()
                 const itemDataTexto = item.texto.toUpperCase()
                  const itemDataCategoria = item.categoria.toUpperCase()
                const campo = itemDataTitle +" "+itemDataTexto+" "+ itemDataCategoria 
                const textData = text.toUpperCase()
                return campo.indexOf(textData) > -1
            })
            this.setState({
                            ...
                            <div className="col-lg-9 mt-5">
                                <div className="row mt-5">
                                    {((this.state.productos && this.state.productos.map(producto => {
                                        ...
                                        ((this.state.carousel && this.state.carousel.map(imagen => {
                                            return (
